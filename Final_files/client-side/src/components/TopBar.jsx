...
const TopBar = ({ isSidebarCollapsed = false }) => {
    ...
    const handleSidebarClick = async (e) => {
      ...
      if (token) {
        try {
          ...
          const uniqueNotifications = unreadNotifications.filter(
  ...
  const cleanupReadNotifications = async () => {
    try {
      ...
      const res = await apiHandler.PostApi(
        api_url.cleanupReadNotifications,
        {},
        token
      );
  ...
  const handleNotificationClick = async (n) => {
    try {
      ...
      if (token && n._id && !n.read) {
        await apiHandler.UpdateApi(
          api_url.markNotifRead + n._id + "/read",
          {},
          token
        );
    ...
    if (category) {
      window.dispatchEvent(
        new CustomEvent("notifications:categoryRead", { detail: { category } })
      );
    }
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={async () => {
                    ...
                    setNotifications([]);
                    window.dispatchEvent(
                      new CustomEvent("notifications:allRead")
                    );
                  }}
                        ...
                        <li key={n._id || n.createdAt}>
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div
                                className={`mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-full ${meta.color}`}
                              >
                                <Icon size={16} />
                              </div>
                              ...
                              {!n.read && (
                                <span className="mt-1 inline-block w-2 h-2 bg-blue-600 rounded-full shrink-0"></span>
                              )}
                            </div>
                          </button>
                        </li>
