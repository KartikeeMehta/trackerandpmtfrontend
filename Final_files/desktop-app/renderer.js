...
// Centralized backend URL (Railway)
const BASE_URL =
  "https://railwayselfpmtdeployed-production.up.railway.app/backend/api";
...
async function fetchStats(email) {
  try {
    const res = await fetch(
      `${BASE_URL}/tracker/stats/today?email=${encodeURIComponent(email)}`
    );
...
async function fetchSessions(email) {
  try {
    const res = await fetch(
      `${BASE_URL}/tracker/sessions/today?email=${encodeURIComponent(email)}`
    );
    const data = await res.json();
      ...
      if (Array.isArray(s.breaks)) {
        for (const b of s.breaks) {
          if (
            b.startedAt &&
            (!lastBreakStart ||
              new Date(b.startedAt) > new Date(lastBreakStart))
          ) {
            lastBreakStart = b.startedAt;
          }
          if (
            b.endedAt &&
            (!lastBreakEnd || new Date(b.endedAt) > new Date(lastBreakEnd))
          ) {
...
// Idle detection: if no mouse/keyboard for 45s, mark idle from 46s onwards
let idleTimer = null;
function userActivity() {
  if (idleSince && Date.now() - idleSince > 46000) {
    pushIdle(new Date(idleSince + 46000), new Date());
  }
  idleSince = Date.now();
  if (idleTimer) clearTimeout(idleTimer);
  idleTimer = setTimeout(() => {
    // goes idle after 45s; actual counted idle starts at 46s (handled above)
  }, 45000);
}
